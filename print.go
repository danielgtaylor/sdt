...
// https://github.com/goccy/go-yaml/blob/master/printer/printer.go
// It now underlines the relevant passage.
package sdt
...
func (p *Printer) setupErrorTokenFormat(annotateLine int) {
...
func (p *Printer) PrintErrorToken(tk *token.Token, length int) string {
	if length == 0 {
		length = 1
	}
	errToken := tk
	...
	p.setupErrorTokenFormat(curLine)
	...
	annotateLine := strings.Repeat(" ", prefixSpaceNum+errToken.Position.Column-1) + strings.Repeat("^", length)
